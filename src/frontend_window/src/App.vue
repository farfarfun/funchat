    ...
    if (data) {
      ...
      if (data.type === "over" && data.payload) {
        ...
        try {
          ...
          if (old_session && old_session.funchat_history === true) {
            sessionLoaded = true;
...
const getSessionDataAsObject = () => {
  ...
  return {
    funchat_history: true, CODE: CODE.value, basic_msg: basic_msg.value, isInit: isInit.value,
...
const checkAndLoadSessionFromFile = async (file) => {
  if (file && file.name.toLowerCase().endsWith('.json')) {
    try {
      ...
      if (jsonData && jsonData.funchat_history === true) {
