...
const feature_suffix = "funchat助手^_^"
...
async function getConfig() {
  ...
  if (configDoc.data && configDoc.data.config && configDoc.data.config.prompts) {
    console.warn("Funchat: Old configuration format detected. Starting migration.");
...
function checkConfig(config) {
  ...
  // --- 3. WebDAV 检查 ---
  if (!config.webdav) {
    config.webdav = { url: "", username: "", password: "", path: "/funchat", data_path: "/funchat_data", localChatPath: "" };
    flag = true;
  } else {
    ...
    const webdavDefaults = { data_path: "/funchat_data", localChatPath: "" };
...
function updateConfig(newConfig) {
  ...
  for (const [code, feature] of featuresMap) {
    if (code === "Funchat Settings" || code === "Resume Conversation") continue;
...
async function savePromptWindowSettings(promptKey, settings) {
  ...
  while (attempt < MAX_RETRIES) {
    ...
    if (result.error && result.name === 'conflict') {
      ...
      attempt++;
      // 为调试记录冲突，但不打扰用户。
      // console.log(`Funchat: DB conflict on saving window settings (attempt ${attempt}/${MAX_RETRIES}). Retrying...`);
    } else {
